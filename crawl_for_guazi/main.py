...
def deal_page(url, car_brand, car_brand_chn, ct, head):
    ...
    对筛选数据页的初步处理，获取
    """
    url1 = f"https://mapi.guazi.com/car-source/carList/pcList?minor={car_brand}&sourceType=&ec_buy_car_list_ab=&location_city=&district_id=&tag=-1&license_date=&auto_type=&driving_type=&gearbox=&road_haul=&air_displacement=&emission=&car_color=&guobie=&bright_spot_config=&seat=&fuel_type=&order=&priceRange=0,-1&tag_types=&diff_city=&intention_options=&initialPriceRange=&monthlyPriceRange=&transfer_num=&car_year=&carid_qigangshu=&carid_jinqixingshi=&cheliangjibie=&key_word={car_brand_chn}&page=1&pageSize=100&city_filter={ct}&city={ct}&guazi_city={ct}&qpres=484192054210134016&versionId=0.0.0.0&osv=IOS&platfromSource=wap"
    ...
    while page <= total_page:
        ...
        page += 1
    print(f"{car_brand_chn} finish! Total is {total}.", end="\n")
...
def get_single_page_info(url, car_brand, car_brand_chn, pg, ct, head):
    ...
    """
    url = url.format(
        car_brand=str(car_brand), car_brand_chn=str(car_brand_chn), pg=pg, ct=ct
    )
    ...
    for single_car in page_text["data"]["postList"]:
        ...
        getAllDataAndSaveToFile(car_brand, single_car["clue_id"], ct)
    # print(df.head(5))
